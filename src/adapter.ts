...
export default async function adapter(
	...
	return {
		...
		supports: {
			read: ({ config, route }) => {
				// Return true if this route can use read() from $app/server
				// You can add logic here to check if specific routes support it
				return true; // or false, or throw an error with instructions
			},

			// Optional: support for instrumentation
