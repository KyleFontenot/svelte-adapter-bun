...
function checkRequestType(obj: Request | IncomingMessage) {
...
const bunViteWSPlugin = (
	...
	const portToUse = process.env?.PUBLIC_DEVWSPORT
		? Number.parseInt(process.env?.PUBLIC_DEVWSPORT)
		: 10234;
	const listeners = {};
	...
	return determineWebSocketHandler({
		...
		return {
			...
			config(config) {
				return {
					server: {
						hmr: { host: "143.197.15.15", protocol: "ws", clientPort: 5173 },
					},
				};
			},
			...
			handleHotUpdate({ file, server }) {
				...
				const isConfigChange = watchFiles.some((configFile) =>
