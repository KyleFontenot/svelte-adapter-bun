...
const bunViteWSPlugin = async (passedOptions: VitePluginOptions): Promise<Plugin> => {
  ...
  return {
    ...
    async configureServer(server: ViteDevServer) {
      ...
      // console.log('bun websocket url:', bunWebSocketServer.url)
      const proxy = createProxyServer({
        target: `${bunserverinst.url}`,
        ws: true,
        changeOrigin: true,
        secure: false,
      });
      // server.httpServer?.on('request', (req, res) => {
      //   console.log('per requert::', req)
      // });
      ...
      server.httpServer?.on('upgrade', async (req: IncomingMessage, socket, head) => {
        ...
        if (isWebSocketUpgrade) {
          try {
            ...
            const targetUrl = `http://localhost:${portToUse}${req.url || ''}`;
            const bunRequest = new Request(targetUrl, {
            ...
            console.log('Converted request:', {
              ...
              headers: Object.fromEntries(bunRequest.headers)
            });

            console.log('HOLED ONE::', bunserverinst)
            // Await the response
            const response = await bunserverinst.fetch(bunRequest, bunserverinst);

            if (response) {
              console.log('Server fetch response:', response);

              // Attempt to upgrade the connection
              if (bunserverinst.upgrade(bunRequest)) {
      ...
      // Cleanup proxy when server is closed
