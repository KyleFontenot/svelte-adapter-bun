...
function sirv_default(_dir, opts = {}) {
  ...
  const gzips = opts.gzip && extensions.map((x2) => `${x2}.gz`).concat("gz");
  const brots = opts.brotli && extensions.map((x2) => `${x2}.br`).concat("br");
  const FILES = {};
...
async function handler_default(assets, websockethandler) {
  ...
  const server2 = new Server(manifest);
  Object.defineProperty(server2, "websocket", websockethandler || defaultAcceptWebsocket);
  await server2.init({ env: (Bun || process).env });
  const handlers = [
  ...
  try {
    const handleWebsocket = server2.websocket();
    if (handleWebsocket) {
      return {
...
var viaLocal = (dir, isEtag, uri, extns) => {
  let i = 0;
  const arr = toAssume(uri, extns);
  let abs;
  let stats;
...
var send = (req, data) => {
  ...
  if (req.headers.has("range")) {
    code = 206;
    const [x2, y] = req.headers.get("range").replace("bytes=", "").split("-");
    opts.end = Number.parseInt(y, 10) || data.stats.size - 1;
    const end = opts.end;
    opts.start = Number.parseInt(x2, 10) || 0;
...
var toHeaders = (name, stats, isEtag) => {
...
installPolyfills();
var xff_depth = Number.parseInt(env("XFF_DEPTH", build_options.xff_depth ?? 1));
